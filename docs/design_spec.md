...
| A4 | 🔜 Next | Monte Carlo FK engine: vectorized batched sampling and chain composition, per-Frame trial-data storage, per-edge RNG sub-stream derivation | `sim/monte_carlo_fk.py` | 8–10 | — |
| A5 | Pending | Post-processing stats: envelope (min/max box), percentile tables, basic histogram data, point-pair relative-transform stats from stored trial data | `postprocess/stats.py` | 6–8 | — |
| A6 | Pending | Hand-verified test cases: 2–3 simple chains (e.g., a 2-edge and a 3-edge chain) checked against manual small-angle calculations; shared-edge consistency regression test; `conftest.py` shared fixtures | `tests/` | 6–8 | — |
| A7 | Pending | Example script(s) / minimal CLI demonstrating end-to-end usage on a real or representative problem, including the multi-chain/shared-frame demonstration | `examples/` | 3–5 | — |
